...
const UserDashboard = () => {
  ...
  const [waterDispensing, setWaterDispensing] = useState(false);
  const [actionError, setActionError] = useState("");
  const [lastWaterDispensed, setLastWaterDispensed] = useState(null);

  // Limpieza inicial de localStorage corrupto
  useEffect(() => {
    const savedTime = localStorage.getItem('lastWaterDispensed');
    if (savedTime) {
      const timestamp = parseInt(savedTime);
      // Solo aceptar timestamps recientes (después de 2020)
      if (timestamp > 1609459200000) { // 1/1/2021
        setLastWaterDispensed(timestamp);
      } else {
  ...
  const formatTime = (milliseconds) => {
    if (!milliseconds || isNaN(milliseconds)) return 'No disponible';
    
    const currentTime = Date.now();
    const diffMs = currentTime - milliseconds;
    
    // Validar que el timestamp sea razonable (no del pasado lejano ni futuro)
    if (diffMs < 0 || diffMs > 31536000000 * 2) { // Más de 2 años
